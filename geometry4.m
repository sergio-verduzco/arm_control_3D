function y = geometry4(c1,c1p,c2,c2p,c3,c3p,c4,c4p,c5,c5p,c6,c6p,c7,c7p, ...
...
% this it produces the muscle lengths, the muscle velocities, velocity 
% errors for each of the 11 muscles, and the 3D vectors along which the 
% muscles exert force.

% The errors produced will be aimed at placing the hand at the specified
...
% This program is used by Arm10c.mdl.

global ShouldIP Tlengths xgain vgain maxlen p1 p2 tension
...
% v_error = vgain*max(x_error + Dtlengths,0); % unsigned error
 v_error = vgain*(x_error + Dtlengths);  % signed error
%v_error = vgain*(x_error + min(Dtlengths,0) + 0.5*max(Dtlengths,0));
