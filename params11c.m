% params11c.m
% Initialize the parameters of the model in Arm11c.mdl (see 4/23/13 entry
...
global ShouldIP maxlen xgain vgain p1 p2 tension last_t e_low ...
       ...
       shoulderQ N3 N7 phi angF pc vd d
...
%          0   0   0   0   0   0   0    0   0  ai   0];
 ...
 MAXNF = 40; % maximum number of features per module
 InpD = 27; % dimensionality of the input vector (without time and IO)
 ...
 if first_sim
     ...
     F = zeros(N3*N7*MAXNF,InpD); % feature matrix
     W = zeros(N3*N7*MAXNF,OutD); % feature weights matrix
 end
 
 TLCS = -0.1*ones(N3+N7,1); % onset time of the last complex spike for module i
 T = 0.03; % time before onset of plasticity [s]
 tauC = 0.02; % anticipation time for feature capture [s] %.02
 %TtauC = T+tauC;
 S = zeros(N3+N7,1); % "unhandled spike" banner
 epC = 0.06; % controls the speed of change in the W weights % .1
 gammaC = 20*ones(N3*N7*MAXNF,1); % controls the width of the feature kernels % 15
 ...
 init = [.9 .2 .2 .2  pi/2 .5*ones(1,11) 0 0 0 1 0 0 0 0 pi 0 10];
 if length(init) == InpD
    states = repmat(init,MAXNst,1); % to save previous input states
 else
     disp('Error: Wrong initial statesize at params11c.m');
 end
