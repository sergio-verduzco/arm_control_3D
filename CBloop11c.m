function y = CBloop11c(u)

% y = CBloop11c(u) implements the cerebellar main loop as described in
% scratch (38). This program is an adaptation of M1RBF2level.m, which
% refines the procedures of CBloop3.m in Arm7.
% The difference with CBloop11b is that we assume a long visual delay, so
...
% u(23) = delayed error (distance from hand to target) [cm]
% u(24) = delayed positive part of error derivative [cm/s]
...
global MAXNF NF F W TLCS gammaC S epC ...
    states Times MAXNst Fis delta shoulderQ N3 N7 phi angF pc vd d
    ...
    for i=1:N3+N7
        if S(i) == 0   % no unhandled spikes from previous steps
            ...
            Pcs = pc*(cos(angF(i)*(u(1) - phi(i)))+1) / ...
                     ( (1+exp(5-u(23)))*(1+exp(30-15*CS)) ); % see (59)        
                     %( (1+exp(7-u(23)))*(1+exp(40-10*CS)) ); % see (59)
            ...
            if CS <= 0  % time to store the feature
                ...
                controls = states((Times > TLCS(i)-vd+d)&(Times < u(1)-vd+d), 6:16);
                meanC = mean(controls);
                base = (i-1)*MAXNF;
                if NF(i) < MAXNF
                    W(base+NF(i),:) = epC*meanC;
                    F(base+NF(i),:) = ...
                        interp1(Times,states,max(Times(1),1.5*TLCS(i)-0.5*u(1)-vd+d));
                    % 1.5*TLCS-0.5*u(1) = TLCS - 0.5*(t - TLCS)
                    disp(['Feature ' num2str(base+NF(i)) ' received weights: ' ...
                        num2str(W(base+NF(i),:))]);
                else
                    FDel = interp1(Times,states,max(Times(1),1.5*TLCS(i)-0.5*u(1)-vd+d));
